--MisDescBegin
x210234_g_ScriptId = 210234
x210234_g_Position_X=160.4355
x210234_g_Position_Z=127.9695
x210234_g_SceneID=2
x210234_g_AccomplishNPC_Name="李工部"
x210234_g_MissionId = 714
x210234_g_Name	="李工部"
x210234_g_MissionKind = 13
x210234_g_MissionLevel = 4
x210234_g_IfMissionElite = 0
x210234_g_IsMissionOkFail = 0		--变量的第0位
x210234_g_DemandItem={{id=20309002,num=1},{id=20309006,num=1}}		--从背包中计算
x210234_g_MissionName="御赐精纺鞋"
x210234_g_MissionInfo="#{event_dali_0047}"  --任务描述
x210234_g_MissionTarget="找到一品桂圆和一品花生，然后回#G大理城五华坛#W找四大善人之一的#R李工部#W#{_INFOAIM160,128,2,李工部}。"		--任务目标
x210234_g_ContinueInfo="  一品桂圆和一品花生你已经找到了？"		--未完成任务的npc对话
x210234_g_MissionComplete="  年轻人，做的不错。"					--完成任务npc说话的话
x210234_g_ItemBonus={{id=10411047,num=1}}
x210234_g_MoneyBonus=0
--MisDescEnd
