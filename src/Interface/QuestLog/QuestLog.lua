...
function QuestLog_ListBox_SelectChanged()
    ...
    if (DataPool:GetPlayerMission_Display(nSelIndex, 7) == 210254) then
        --如果任务处理脚本是210254（老带新活动）
...
--  任务追踪复选框点击回调
function QuestLog_MissionTrack_Clicked(nIndex)
    Clear_XSCRIPT();
    Set_XSCRIPT_Function_Name("updateMCTrackerChecked");
    Set_XSCRIPT_ScriptID(45);
...
function QuestLog_ShowWindow()
    this:TogleShow();
    local nMissionTrackShow = DataPool:GetPlayerMission_DataRound(185) --MD_MISSION_TRACK_FLAG
    local nCampaignTrackShow = DataPool:GetPlayerMission_DataRound(186) --MD_CAMPAIGN_TRACK_FLAG
    if (nMissionTrackShow or -1) > 0 then
        QuestLog_Mode1:SetCheck(1);
    else
        QuestLog_Mode1:SetCheck(0);
    end
    if (nCampaignTrackShow or -1) > 0 then
