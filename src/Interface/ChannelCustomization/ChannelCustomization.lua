...
function ChannelCustomization_OnLoad()
	CUSTOM_BUTTONS[1] = ChannelCustomization_VicinityInfo;        --附近
	CUSTOM_BUTTONS[2] = ChannelCustomization_TeamInfo;            --队伍
	CUSTOM_BUTTONS[3] = ChannelCustomization_WorldInfo;            -- 世界
	CUSTOM_BUTTONS[4] = ChannelCustomization_PersonalInfo;        --个人
	CUSTOM_BUTTONS[5] = ChannelCustomization_SystemInfo;        --系统频道
	CUSTOM_BUTTONS[6] = ChannelCustomization_SelfInfo;            --自建频道（现在没有功能）
	CUSTOM_BUTTONS[7] = ChannelCustomization_CorporateInfo;        --帮会
	CUSTOM_BUTTONS[8] = ChannelCustomization_MenpaiInfo;        --门派
	CUSTOM_BUTTONS[9] = ChannelCustomization_SelfInfo;            --个人信息
...
function ChatTabAction(act, strCfg, strName, pos)
	...
	if (string.byte(strCfg, 12) == 49) then
		...
		for i = 1, 13 do
			if i ~= 12 then
				if (string.byte(strCfg, i) == 48) then
					-- 0
					CC_BUTTON_SetCheck(i, 0);
				elseif (string.byte(strCfg, i) == 49) then
					-- 1
					CC_BUTTON_SetCheck(i, 1);
				end
				CC_BUTTON_Enable(i);
