...
function PetFrame_Refrensh()
	Clear_XSCRIPT()
		Set_XSCRIPT_Function_Name( "AskRefreshPet" );
		Set_XSCRIPT_ScriptID( 900009 );	
		Set_XSCRIPT_ParamCount(0);
	Send_XSCRIPT()
	KillTimer("PetFrame_Refrensh()")
end
...
function PetFrame_OnEvent( event )
	if ( event == "PLAYER_ENTERING_WORLD" ) then
		--this:Show();
		PetFrame_Update();
		SetTimer("PetFrame","PetFrame_Refrensh()", 4000)
		return;
...
function PetFrame_Update()
    ...
    if nPetMaxHp ~= 0 then
	    PET_MAXHP = PET_MAXHP + nPetMaxHp
	    PET_HP = PET_HP + nPetMaxHp
	end
