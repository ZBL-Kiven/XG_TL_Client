...
function MiniMap_OnEvent(event)
	...
	if (event == "UI_COMMAND" and tonumber(arg0) == 20201115) then
		--7日豪礼检测，如果签到超过7天。就隐藏图标了。
...
function MiniMap_UpdateLockFlag()
	...
	if (Player:IsHavePassword() == 0) then
		--如果有没有密码呢，就闪烁
...
function Minimap_UpdatePKMode()
	...
	if (tonumber(nPKMode) == 0 or tonumber(nPKMode) == 5) then
...
function MiniMap_FeelFeedBackBtnClk()

end
function MiniMap_XinShouNewBtnClk()
	Clear_XSCRIPT();
	Set_XSCRIPT_Function_Name("OpenServerDay");
